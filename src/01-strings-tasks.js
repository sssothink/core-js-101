...
function extractNameFromTemplate(value) {
  const delStr = value.split(' ');
  delStr.shift();
  return delStr.join(' ').slice(0, -1);
}
...
function removeLeadingAndTrailingWhitespaces(value) {
  ...
  if (value.split('')[value.length - 1] === ' ' || value.split('')[value.length - 1] === '\t') {
    for (let i = newStr.length; i > 0; i -= 1) {
      if (newStr[i] !== ' ' && newStr[i] !== '\t') {
        newStr.splice(i - 1);
        break;
      }
  ...
  for (let i = 0; i < 13; i += 1) {
    for (let k = 0; k < newStr.length; k += 1) {
      if (newStr[k] === a) {
        ...
        newStr[k] = `${a}`;
      } else if (newStr[k] === n.toUpperCase()) {
        newStr[k] = `${a.toUpperCase()}`;
      } else if (newStr[k] === a.toUpperCase()) {
        newStr[k] = `${n.toUpperCase()}`;
      }
    }
...
function isString(value) {
  if (value !== null && String(value) !== '' && typeof value === 'object') {
    return true;
  }
